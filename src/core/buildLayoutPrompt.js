...
- Decide the composition_strategy. It MUST be exactly one of these values (no others exist):
  full_image | image_above_text | text_above_image | image_left_text_right | text_left_image_right |
  equal_images | hero_support | grid_gallery | gallery_left_text_right | images_spread_across_pages |
  column_flow_grid
  **PRIORITY (flexible layout first)**:
  1. image_left_text_right / text_left_image_right / image_above_text / text_above_image — PRIMARY (use these as your default for images+text pairs, varying which one per page for visual rhythm)
  2. hero_support / equal_images — when comparing or hero-featuring specific images
  3. grid_gallery / images_spread_across_pages — when 3+ images benefit from distribution across pages
  4. ⚠️ column_flow_grid — FALLBACK ONLY (use only if nothing above fits)
  🚫 gallery_page_text_page does not exist as a valid value -- never output it.
  🚨 Never use column_flow_grid as your first/default choice when images+text are both present.
...
- The text element's "role" field MUST be exactly one of these values (no others exist):
  title | subtitle | body | section_label | page_number | continuation_body
...
- 🔴 CRITICAL: the top-level "grid": {columns, rows} object's columns/rows MUST be numerically identical to grid_spec.columns/grid_spec.rows if you include grid_spec. If you don't include grid_spec, "grid" MUST be {columns: 6, rows: 12} exactly. A mismatch fails validation.
- 🔴 CRITICAL: layout_purpose and image_text_relation are REQUIRED fields on every candidate (see allowed values in "Your allowed decisions" above). Omitting either fails validation.
- 🔴 CRITICAL span variation (fails validation if violated): within a single candidate, do NOT give every text element the same col_span, and do NOT give every image element the same col_span. Mix at least two different col_span values across your text elements, and if there are 2+ images, mix at least two different col_span values across your images too. Reusing one fixed column width for everything is exactly the "rigid grid" mistake this system exists to avoid.
- 🔴 CRITICAL vertical spacing (fails validation if violated): columns already have a built-in gutter, but ROWS DO NOT. Two elements stacked on top of each other (one's row_start immediately follows the other's row_start+row_span) will be flagged as touching/colliding. ALWAYS leave at least 1 empty row between two elements that are vertically stacked in the same column range (e.g. if element A occupies row_start=1, row_span=5 (rows 1-5), the next element below it in the same columns must start at row_start=7 or later, never row_start=6).
